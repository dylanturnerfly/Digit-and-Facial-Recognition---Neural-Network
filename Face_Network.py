...
directory = './network_weights/stored_weights_face' #path to weights directory
...
input_hidden_weights = np.random.uniform(-0.5 , 0.5, (hidden_layer_height , 4200))
hidden_output_weights = np.random.uniform(-0.5 , 0.5, (1 , hidden_layer_height))
input_hidden_bias = np.zeros((hidden_layer_height,1))
hidden_output_bias = np.zeros((1 , 1))
...
def print_face(faces, index):
    if index < len(faces):
        face = np.reshape(faces[index],(70,60))
        for row in face:
    ...
    for face, label in zip(faces, labels):
       # label.shape += (1,)
        ...
        if output >= 0.5 and label == 1:
            ...
            correct += 1
    # print(str(round(correct / len(faces), 6)) + " accuracy")
...
def createImage():
    ...
    sns.heatmap(np.reshape(deltaI, (70,60)), annot=False, cmap='Greys', cbar=True)
    plt.title('Neural Network Weights Visualization for Face:\n' + str(epochs) + " Epochs, " + str(learnRate) + " Learning Rate")
    plt.show()
...
#Load data once
training_faces = read_faces_from_file("./data/facedata/facedatatrain.txt")
training_labels = read_labels("./data/facedata/facedatatrainlabels.txt")
testing_faces = read_faces_from_file("./data/facedata/facedatatest.txt")
testing_labels = read_labels("./data/facedata/facedatatestlabels.txt")
...
if(from_scratch == 'y'):
    ...
    for i in range(percent_loops):
        ...
        for j in range(Trials):
            ...
            x,y = sample_percentage_two_arrays(training_faces, training_labels, i * choice * 100) #shuffled and sampled
        ...
        print("\n-------------------------------------------------------------------")
        print(str(Trials) + " Trials, " + str(epochs) + " Epochs, " + str(learnRate) + " learning rate, " + str(round(i * choice * 100,3)) 
            + " percent of data.") 
        print("-------------------------------------------------------------------")
...
if(full_training_done): #only plot if they trained through every percentage
    ...
    if(show_plot == 'y'):
        ...
        #Average Accuracy plot and standard deviation plot
        plt.figure()
        ...
        #plot weight map here
        createImage()

#Tests trained perceptron on specified index
def test_one(index):
    result = testOneImage(testing_faces[index])
    print("Network Guess: " + str(result) + "   Correct Label: " + ("True" if testing_labels[index] == 1 else "False"))
    
#Promt user for request
if(training_done):
    cont = True
    while(cont):
        index = input("\nWhich image to test? (0-149, s to exit): ")
        if(index == "s"):
            cont = False
        else:
